...
namespace Fatec.RD.Infra.Repositorio.Base
{
    public class RelatorioDespesaRepositorio
    {
        ...
        /// <summary>
        /// Método que deleta a relação entre Despesa e Relatório
        /// </summary>
        /// <param name="idDespesa"></param>
        /// <param name="idRelatorio"></param>
        public void Deletar(int idDespesa, int idRelatorio)
        {
            var sqlCommand = @"DELETE FROM RelatorioDespesa WHERE IdDespesa = @IdDespesa AND IdRelatorio = @IdRelatorio";
            _connection.Execute(sqlCommand, new { IdDespesa = idDespesa, IdRelatorio = idRelatorio });
        }
